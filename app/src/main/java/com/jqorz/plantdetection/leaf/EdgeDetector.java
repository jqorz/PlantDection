...
public class EdgeDetector {
    ...
    private static Bitmap sourceImage;                //源图片
     ...
     * canny算法
     */
    private static void canny_core(float f, int i) {
    ...
    private static int[] image2pixels(Bitmap image) {
        int ai[] = new int[picsize];
        //转换为像素数组
        ColorUtil.getPixels(image, 0, 0, width, height, ai);
        boolean flag = false;
        int k1 = 0;
        do {
            if (k1 >= 16)
    ...
    public static void setSourceImage(Bitmap image) {
        sourceImage = image;
    }
    ...
    public static double[] getImage(Bitmap image) {
        new EdgeDetector();
